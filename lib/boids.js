...
{
    cx.strokeStyle = "rgba(86,212,255,.35)";
    cx.lineWidth = 1;

    cx.beginPath();
    cx.arc(mousePosition.x, mousePosition.y, 12, 0, Math.PI*2);
    cx.stroke();

    cx.beginPath();
    cx.moveTo(mousePosition.x - 18, mousePosition.y);
    cx.lineTo(mousePosition.x + 18, mousePosition.y);
    cx.moveTo(mousePosition.x, mousePosition.y - 18);
    cx.lineTo(mousePosition.x, mousePosition.y + 18);
    cx.stroke();
}
...
function drawGrid(size = 40)
{
    cx.strokeStyle = THEME.grid;
    cx.lineWidth = 1;
    cx.globalAlpha = 0.45;
...
function renderText(context, position, text, font, color)
{
    ...
    context.font = font || "14px Inter, Arial";
    ...
    {
        context.fillText(lines[i], position.x, position.y + i * 16);
    }
}
...
class Transform {
    ...
    render() {
        drawArrow(cx, this.position, this.position.clone().add(this.forward().scale(10)), 5, THEME.red);
        drawArrow(cx, this.position, this.position.clone().add(this.right().scale(10)), 5, THEME.blue);
    }
}
    ...
    render() {
        ...
        for (let x=0; x < this.densityX; x++) {
            for (let y=0; y < this.densityY; y++) {
                ...
                drawArrow(cx, position, position.clone().add(force.clone().scale(0.5)), 3, colorString);
            }
        }
...
class Swarmer {
    ...
    render() {
        ...
        cx.beginPath();
        cx.fillStyle = THEME.orange;

        if (this.length > 0)
        {
            const grad = cx.createLinearGradient(positionBegin.x, positionBegin.y, positionEnd.x, positionEnd.y);
            grad.addColorStop(0, THEME.orange);
            grad.addColorStop(1, THEME.yellow);
            cx.fillStyle = grad;
        }

        cx.shadowBlur = 12;
        cx.shadowColor = "rgba(255,209,102,.35)";
        ...
        cx.fillStyle = "#fff";
        cx.beginPath();
        cx.arc(positionBegin.x, positionBegin.y, 2, 0, Math.PI * 2);
        cx.fill();
        ...
        if (gameState.options.simulation.debugForces) {
            ...
            drawArrow(cx, this.transform.position, this.transform.position.clone().add(this.finalForce), 5, THEME.orange);
...
function init() {
    ...
    canvas.addEventListener("touchmove", e=>{
        ...
        mousePosition.x = (t.clientX - rect.left) * (800 / rect.width);
        mousePosition.y = (t.clientY - rect.top) * (800 / rect.height);
